...
# Fit a Mean Forecast model to the historical data
mean_forecast <- meanf(stock_ts, h = forecast_period)

# Plot the forecast
plot(mean_forecast, main = paste(symbol, " Stock Price Forecast"), xlab = "Date", ylab = "Price", type = "l", col = "red")
lines(stock_ts, col = "blue", lwd = 2)

# Add a legend
legend("topright", legend = c("Mean Forecast", "Historical"), col = c("red", "blue"), lty = 1, lwd = 2)

# Print the forecast result
print(mean_forecast$mean)

# Calculate forecast accuracy measures
accuracy(mean_forecast$mean, stock_ts)
