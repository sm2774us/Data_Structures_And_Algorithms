...
arima_model <- arima(closing_prices_numeric)  # Adjust order as needed

# Forecast future prices using the AR model
forecast_values <- predict(arima_model, n.ahead = 100)  # Fix the variable name here
...
if (sd(forecast_values$pred) == 0) {
  ...
  adjusted_r2 <- 1 - (1 - r2) * (length(actual_values) - 1) / (length(actual_values) - length(arima_model$ar))  # Fix the variable name here
}
