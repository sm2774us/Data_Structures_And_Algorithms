...
volume_max <- max(Vo(Stock), na.rm = TRUE)
volume_min <- min(Vo(Stock), na.rm = TRUE)
...
# Create the volume moving average plot (bottom)
volume_plot <- ggplot(data = Stock_df, aes(x = Date)) +
  ...
  geom_hline(yintercept = volume_max, color = "darkred", linetype = "dashed", size = 1, label = "Volume Max") +
  geom_hline(yintercept = volume_min, color = "darkgreen", linetype = "dashed", size = 1, label = "Volume Min") +
